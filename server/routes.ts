...
<body>
  <div class="container">
    <div class="header">
      <div class="logo">Arica Toucan</div>
    ...
    <div class="footer">
      <p>Generated by Arica Toucan on ${new Date().toLocaleDateString()}</p>
  ...
  app.get("/api/download/agent", async (req, res) => {
    try {
      ...
      const agentPath = path.join(process.cwd(), "desktop-agent", "arica_toucan_agent.py");
      const batPath = path.join(process.cwd(), "desktop-agent", "AricaToucanAgent.bat");
      ...
      res.setHeader('Content-Disposition', 'attachment; filename="AricaToucanAgent.zip"');
      
      const archive = archiver.default('zip', { zlib: { level: 9 } });
      archive.pipe(res);
      
      archive.file(agentPath, { name: 'arica_toucan_agent.py' });
      if (fs.existsSync(batPath)) {
        archive.file(batPath, { name: 'AricaToucanAgent.bat' });
      }
